...
文档：[English](README.md) | 简体中文 | [Español](README.es.md)

适用于已经部署的自托管 n8n，提供简体中文（`zh-CN`）、繁體中文（`zh-TW`）和西班牙语（`es`）界面语言包。本项目只提供界面翻译和可卸载安装器，不包含、不安装、也不重新分发 n8n 本体。

## 可用界面语言包

| 语言代码 | 语言 | 状态 | 完整适配基线 |
| --- | --- | --- | --- |
| `zh-CN` | 简体中文 | 已完成 | n8n 2.34.6 |
| `zh-TW` | 繁體中文（台灣） | 预览 | n8n 2.34.6 |
| `es` | 西班牙语 | 预览 | n8n 2.34.6 |
...
- 以 n8n 2.34.6 为完整适配基线。其他版本仅翻译能够精确匹配的文案。
